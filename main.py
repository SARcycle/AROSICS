...
def main():
    ...
    for date in dates:
        try:
            ...
            print(str)
            print("-" * len(str))
            print(f'AROSICS started at {datetime.datetime.now().strftime("%Y/%m/%d %H:%M:%S")}')
            ...
            for acquisitions in counts.items():
                ...
                s2_b04_tiles = glob.glob(
                    os.path.join(os.path.join(base_path, base_path_suffix), f'R{relative_orbit:03}',
                                 f'{acquisition_date.strftime("%Y%m%d")}', '*_B04_10m.jp2'))
                s2_cloud_tiles = glob.glob(
                    os.path.join(os.path.join(base_path, base_path_suffix), f'R{relative_orbit:03}',
                ...
                if len(s2_b04_tiles) == tile_count and len(s2_cloud_tiles) == tile_count:
                    if mosaicing:
                        ...
                        out_path_dx = coreg_main.coregister_S2(b04_file_path_tmp, cld_file_path_tmp,
