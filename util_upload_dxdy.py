...
def determine_run_type():
    ...
    config_GDRIVE_SECRETS = os.path.join('secrets', 'geetest-credentials.secrets')
    config_GDRIVE_SECRETS = '/mnt/c/Users/Localadmin/Documents/SATROMO/AROSICS_Coregistration/AROSICS/secrets/geetest-credentials.secret'  # Hackjob, since the previous approach does not work
    ...
    if os.path.exists(config_GDRIVE_SECRETS):
        ...
        gauth.service_account_file = config_GDRIVE_SECRETS
...
def upload_dx_dy_mosaic_for_single_date(day_to_process: str, collection: str) -> None:
    ...
    run_type, storage_client = determine_run_type()
    ...
    try:
        ...
        print(f'Upload finished at {datetime.now().strftime("%Y/%m/%d %H:%M:%S")}')
        # delete file on GCS
